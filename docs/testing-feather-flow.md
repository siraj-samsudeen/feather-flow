# feather-flow — Independent Test Plan

You are testing **feather-flow**, a config-driven Python ETL tool that extracts data from source databases and files into a local DuckDB warehouse.

**Before writing any test:** Read this plan top to bottom. Each section builds on the previous one.

---

## How feather-flow Works (Read This First)

feather-flow reads a `feather.yaml` config file that declares source databases/tables and where to load them. It extracts data from the source, loads it into a local DuckDB file, and tracks what it has done in a separate state database.
...
Run these commands first. They show you what feather-flow does and how it works. Every other test goes through these commands.
...
Change detection determines whether feather-flow re-extracts a table or skips it. This directly affects performance and correctness.
...
feather-flow has three load strategies. Each handles re-extraction differently.
...
On each extraction, feather-flow compares the source table's current schema against the stored snapshot in `_schema_snapshots`. On the first run, it saves a baseline. On subsequent runs:
...
When `source_table` contains a glob pattern (e.g., `sales_*.csv`), feather-flow combines all matching CSV files into a single table. All files must have the same schema (column names and types). Change detection tracks per-file mtime + hash.
