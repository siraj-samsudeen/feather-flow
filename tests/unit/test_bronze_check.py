...
def _silver(name: str, sql: str = "SELECT 1") -> TransformMeta:
...
def _gold(name: str, sql: str = "SELECT 1") -> TransformMeta:
    return TransformMeta(
        name=name,
        schema="gold",
        sql=sql,
...
class TestCheckBronzeDependencies:
    ...
    def test_one_declared_dep_present_returns_empty(
        ...
        transforms = [_silver("orders", sql="SELECT * FROM bronze.orders")]
        assert check_bronze_dependencies(con, transforms) == []
    ...
    def test_six_missing_deps_returns_six_warnings(
        ...
        transforms = [
            _silver(f"t{i}", sql=f"SELECT * FROM bronze.t{i}") for i in range(6)
        ]
        warnings = check_bronze_dependencies(con, transforms)
    ...
    def test_mixed_with_and_without_depends_on(
        ...
        transforms = [
            _silver("no_deps"),
            _silver("ok", sql="SELECT * FROM bronze.present"),
            _silver("missing", sql="SELECT * FROM bronze.absent"),
        ]
        warnings = check_bronze_dependencies(con, transforms)
        assert len(warnings) == 1
    ...
    def test_silver_dep_on_silver_not_checked(
        ...
        # A silver→silver reference is a cross-transform dep, not a bronze
        # dep. check_bronze_dependencies walks bronze references only and
        # must ignore it (no warning, no false positive even though
        # silver.x doesn't exist in the destination).
        transforms = [_silver("derived", sql="SELECT * FROM silver.x")]
        assert check_bronze_dependencies(con, transforms) == []
    ...
    def test_gold_with_bronze_dep_not_checked(
        ...
        # A gold transform that (improperly) references bronze is out of
        # scope. This guards against scope creep.
        transforms = [_gold("g", sql="SELECT * FROM bronze.absent")]
        assert check_bronze_dependencies(con, transforms) == []
